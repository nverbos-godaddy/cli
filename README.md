...
| [Run Tests on an Authorization Model](#run-tests-on-an-authorization-model) | `test`      | `--tests`, `--verbose`, `--server-config` | `fga model test --tests "**/*.fga.yaml"`                                          |
...
* `--server-config`: Path to a YAML file with configuration for the built-in OpenFGA server used during local testing. Uses the same property names as the [OpenFGA server configuration](https://openfga.dev/docs/getting-started/setup-openfga/configuration). Supported properties: `maxTypesPerAuthorizationModel`, `maxAuthorizationModelSizeInBytes`, `maxTuplesPerWrite`, `resolveNodeLimit`, `resolveNodeBreadthLimit`, `listObjectsDeadline`, `listObjectsMaxResults`, `listUsersDeadline`, `listUsersMaxResults`, `requestTimeout`.
